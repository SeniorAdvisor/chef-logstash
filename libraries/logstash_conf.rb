  ...
  def self.section_to_str(section, version = nil, patterns_dir = nil, indent = 0)
    ...
    section.each do |segment|
      ...
      if segment.key?('condition') || segment.key?('block')
        ...
        if segment['block'].is_a?(::Array)
          segment['block'].each do |seg|
            result << plugin_to_arr(seg, patterns_dir_plugins, patterns_dir, indent)
          end
        else
        ...
        if segment.is_a?(::Array)
          segment.each do |seg|
            result << plugin_to_arr(seg, patterns_dir_plugins, patterns_dir, indent) # , type_to_condition)
          end
        else
